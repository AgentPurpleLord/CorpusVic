...
def test_add_link_persists_and_returns_a_record(isolate_links):
    ...
    record = add_link("crimes-act", "s5", start=start, end=end, label="act_citation", node_text=node_text)
    assert record["node_id"] == "s5"
...
def test_add_link_rejects_unknown_label(isolate_links):
    with pytest.raises(LinkError, match="Unknown label"):
        add_link("crimes-act", "s0", start=0, end=3, label="not-a-real-label", node_text="abc")
...
def test_add_link_rejects_out_of_range_spans(isolate_links, start, end):
    with pytest.raises(LinkError, match="out of range"):
        add_link("crimes-act", "s0", start=start, end=end, label="other", node_text="short text")
    assert load_links("crimes-act") == []


def test_add_link_keeps_appending_to_the_same_act(isolate_links):
    add_link("crimes-act", "s0", start=0, end=3, label="other", node_text="one two")
    add_link("crimes-act", "s1", start=0, end=3, label="defined_term", node_text="two three")
    assert len(load_links("crimes-act")) == 2


def test_add_link_keeps_different_acts_separate(isolate_links):
    add_link("crimes-act", "s0", start=0, end=3, label="other", node_text="one two")
    add_link("evidence-act", "s0", start=0, end=3, label="other", node_text="one two")
...
def test_delete_link_removes_the_matching_record(isolate_links):
    a = add_link("crimes-act", "s0", start=0, end=3, label="other", node_text="one two")
    b = add_link("crimes-act", "s1", start=0, end=3, label="defined_term", node_text="two three")
...
def test_save_links_replaces_this_acts_links_wholesale(isolate_links):
    ...
    replacement = {
        "id": "kept", "node_id": "s9", "node_index": None, "start": 0, "end": 3, "text": "abc",

"""Chunks extracted page text and asks a backend (local Ollama model or
Claude -- see llm_backend.py) to structure each chunk into hierarchical
legislative components, then stitches chunks back together."""
...
def structure_act(pages: list[PageText], act_name: str, backend, pages_per_chunk: int = 8, verbose: bool = True) -> list[dict]:
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        nodes = backend.parse_chunk(text, system_prompt)
